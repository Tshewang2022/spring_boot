package tshewang.kafkaconsumer;
...
public class KafkaconsumerApplication {
	...
	private static KafkaConsumer<String, String> createKafkaConsumer() {
		...
		// this setting will not allow to commit auto offset auto
	...
	public static void main(String[] args) throws IOException {
		Logger log = LoggerFactory.getLogger(OpenSearchClient.class.getSimpleName());
...
//					String id = record.topic() + "_" +record.partition() + "_" +record.offset();
				...
				if(bulkRequest.numberOfActions()>0){
					BulkResponse bulkReponse =openSearchClient.bulk(bulkRequest, RequestOptions.DEFAULT);
					log.info("Inserted"+bulkReponse.getItems().length+ "record(s)");
					try{
						Thread.sleep(1000);
					}catch (InterruptedException e){
						e.printStackTrace();
					}
					// comit offsets after the batch is consumed
					consumer.commitAsync();
					log.info("Offset have been committed manaually!");
				}
			}
			...
			log.info("The consumer is not gracefully shut down");
		}
	}
